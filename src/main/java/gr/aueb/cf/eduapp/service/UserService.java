...
public class UserService implements IUserService {
    ...
    public UserReadOnlyDTO saveUser(UserInsertDTO userInsertDTO) throws EntityAlreadyExistsException, EntityInvalidArgumentException {
        try {
            ...
            Role role = roleRepository.findById(userInsertDTO.roleId())
                    .orElseThrow(() -> new EntityInvalidArgumentException("Role", "Role with id=" + userInsertDTO.roleId() + "is wrong"));
            role.addUser(user);
            userRepository.save(user);
            log.info("User with username ={} saved succesfully", userInsertDTO.username());
            ...
            log.error("Save failed. Invalid arguments gor user with username ={}", userInsertDTO.username());
            throw e;
        }
    ...
    public UserReadOnlyDTO getUserByUUID(UUID uuid) throws EntityNotFoundException {
        ...
        try {
            ...
            log.debug(" User with uuid ={} found succesfully", uuid);
            return mapper.mapToUserReadOnlyDTO(user);
